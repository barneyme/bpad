document.addEventListener("DOMContentLoaded", function () {
    ...
    function storeLocally() {
        localStorage.setItem("bedit_" + tabId, elements.textbox1.value);
    }
    ...
    function updateLinkButtonState() {
        ...
        while ((match = urlRegex.exec(currentLineText)) !== null) {
            ...
            if (cursorIndexInLine >= urlStartIndexInLine && cursorIndexInLine <= urlEndIndexInLine) {
                foundLink = match[0];
                break;
            }
        }
    ...
    function handleImportSettings(event) {
        ...
        reader.onload = (e) => {
            try {
                ...
                alert("Error: Could not parse settings file. Please make sure it's a valid JSON file.");
    ...
    function openHighlightedLink() {
        ...
        const selectedText = textbox.value.substring(textbox.selectionStart, textbox.selectionEnd).trim();
        let urlToOpen = null;

        if (selectedText) {
            urlToOpen = selectedText;
        } else if (currentLinkUrl) {
            urlToOpen = currentLinkUrl;
        }
        ...
        try {
            ...
            console.error("The text is not a valid URL:", e);
        }
    }
    ...
    function handleKeyDown(event) {
        ...
        if (event.key === "Enter") {
            ...
            if (settings.autoIndent) {
                ...
                newCursorPos = s + 1 + indentation.length;
            } else {
                newText = text.substring(0, s) + '\n' + text.substring(e);
                newCursorPos = s + 1;
            }
            ...
            targetTextbox.selectionStart = targetTextbox.selectionEnd = newCursorPos;
            updateAllUI();
            return;
        }
        ...
        if (event.ctrlKey || event.metaKey) {
            switch (event.key.toLowerCase()) {
                ...
                case "k": event.preventDefault(); openHighlightedLink(); break;
                ...
                case "g": event.preventDefault(); togglePopup("settings", true); break;
            }
        }
    ...
    elements.settingsBtn.addEventListener("click", () => togglePopup("settings", true));
